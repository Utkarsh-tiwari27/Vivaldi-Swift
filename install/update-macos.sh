#!/usr/bin/env bash
#
# Vivaldi Swift — macOS Auto-Updater
# ----------------------------------------------------------------------------
# This is the background service the installer registers as a LaunchAgent.
# It is not meant to be run manually — install once, forget it exists.
# Whenever the repository changes, Vivaldi Swift quietly updates itself;
# there is no manual update command to run.
#
...
#
# Usage:
#   ./update-macos.sh [--force] [--quiet] [--app-path <path>]
#
# Exit codes:
...
Options:
  ...
  --app-path <path>      Explicit path to Vivaldi.app, passed through to
                          the patch engine when reapplying
...
if ! "$MOD_DIR/bin/patch-macos.sh" "${PATCH_ARGS[@]}"; then
    log ERROR "Patch reapplication failed. See $MOD_DIR/logs/patch-macos.log for details."
    exit 3
fi

log INFO "=== update-macos.sh finished ==="
